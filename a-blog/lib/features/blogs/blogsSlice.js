...
function nextBlogId(blogs) {
    const maxId = blogs.reduce((maxId, blog) => Math.max(blog.id, maxId), -1)
    return maxId + 1
  }
...
export default function blogsReducer(state = initialState,action){
    ...
    switch (action.type) {
      case "set_blogs":
        return action.posts;
      case "add_blog":
        const { title, blogbody, author, img } = action.blog;
        return {
            ...state,
            blogs:[...state.blogs,{ title, blogbody, author, img, id: nextBlogId(state.blogs) } ]
            }
